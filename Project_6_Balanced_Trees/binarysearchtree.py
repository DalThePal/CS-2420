...
class BinarySearchTree:
    ...
    def inorder(self):
        ...
        while len(lyst) < length:
            data = self.__inorder_helper(self.root)
            lyst.append(data)
            self.remove(data)

        return lyst

    def __inorder_helper(self, cursor):
        '''recursion helper for inorder'''

        if cursor.left_child:
            return self.__inorder_helper(cursor.left_child)

        else:
            return cursor.data

    def rebalance_tree(self):
        '''do an inorder traversal of the tree and write the node values out to a list'''
        ordered_lyst = self.inorder()

        self.root = self.__rebalance_helper(ordered_lyst)

        return

    def __rebalance_helper(self, lyst):
        ...
        cursor = Node(middle)
        cursor.left_child = self.__rebalance_helper(left)
        cursor.right_child = self.__rebalance_helper(right)
